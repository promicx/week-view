...
import com.promicx.weekview.WeekViewUtil.isSameDay
import com.promicx.weekview.WeekViewUtil.today
...
class WeekView @JvmOverloads constructor(private val mContext: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(mContext, attrs, defStyleAttr) {
    ...
    private val mGestureListener = object : GestureDetector.SimpleOnGestureListener() {
        ...
        override fun onScroll(e1: MotionEvent, e2: MotionEvent, distanceX: Float, distanceY: Float): Boolean {
            ...
            if (mCurrentScrollDirection == Direction.NONE) {
                ...
                if (abs(distanceX) > Math.abs(distanceY) && distanceX < -mScaledTouchSlop) {
                    mCurrentScrollDirection = Direction.RIGHT
                }
        ...
        override fun onFling(e1: MotionEvent, e2: MotionEvent, velocityX: Float, velocityY: Float): Boolean {
            ...
            if (mCurrentFlingDirection == Direction.LEFT || mCurrentFlingDirection == Direction.RIGHT) mScroller!!.fling(mCurrentOrigin.x.toInt(), mCurrentOrigin.y.toInt(), (velocityX * xScrollingSpeed).toInt(), 0, Integer.MIN_VALUE, Integer.MAX_VALUE, (-((mHourHeight * 24).toFloat() + mHeaderHeight + (mHeaderRowPadding * 2).toFloat() + mHeaderMarginBottom + mTimeTextHeight / 2 - height)).toInt(), 0)
            else if (mCurrentFlingDirection == Direction.VERTICAL) mScroller!!.fling(mCurrentOrigin.x.toInt(), mCurrentOrigin.y.toInt(), 0, velocityY.toInt(), Integer.MIN_VALUE, Integer.MAX_VALUE, (-((mHourHeight * 24).toFloat() + mHeaderHeight + (mHeaderRowPadding * 2).toFloat() + mHeaderMarginBottom + mTimeTextHeight / 2 - height)).toInt(), 0)
    ...
    private fun drawTimeColumnAndAxes(canvas: Canvas) {
        ...
        clipRect(canvas, 0f, mHeaderHeight + mHeaderRowPadding * 2, mHeaderColumnWidth, height.toFloat())
    ...
    private fun drawHeaderRowAndEvents(canvas: Canvas) {
        ...
        if (mAreDimensionsInvalid) {
            mEffectiveMinHourHeight = max(mMinHourHeight, ((height.toFloat() - mHeaderHeight - (mHeaderRowPadding * 2).toFloat() - mHeaderMarginBottom) / 24).toInt())
        ...
        val leftDaysWithGaps = (-ceil((mCurrentOrigin.x / (mWidthPerDay + mColumnGap)).toDouble())).toInt()
        ...
        clipRect(canvas, mHeaderColumnWidth, mHeaderHeight + (mHeaderRowPadding * 2).toFloat() + mHeaderMarginBottom + mTimeTextHeight / 2, width.toFloat(), height.toFloat())
        ...
        for (dayNumber in leftDaysWithGaps + 1..leftDaysWithGaps + mNumberOfVisibleDays + 1) {
            ...
            if (mWidthPerDay + startPixel - start > 0) {
                if (mShowDistinctPastFutureColor) {
                    ...
                    canvas.drawRect(start, mHeaderHeight + (mHeaderRowPadding * 2).toFloat() + mTimeTextHeight / 2 + mHeaderMarginBottom, startPixel + mWidthPerDay, height.toFloat(), if (sameDay) mTodayBackgroundPaint else mDayBackgroundPaint)
                }
            }
        ...
        clipRect(canvas, 0f, 0f, mTimeTextWidth + mHeaderColumnPadding * 2, mHeaderHeight + mHeaderRowPadding * 2)
        ...
        clipRect(canvas, mHeaderColumnWidth, 0f, width.toFloat(), mHeaderHeight + mHeaderRowPadding * 2)
        ...
        if (mEventRects != null && mEventRects!!.size > 0) {
            for (i in mEventRects!!.indices) {
                if (isSameDay(mEventRects!![i].event.startTime, date) && !mEventRects!![i].event.isAllDay) {
                    ...
                    // Calculate top.
                    val top = mHourHeight.toFloat() * 24f * mEventRects!![i].top / 1440 + mCurrentOrigin.y + mHeaderHeight + (mHeaderRowPadding * 2).toFloat() + mHeaderMarginBottom + mTimeTextHeight / 2 + mEventMarginVertical.toFloat()

                    // Calculate bottom.
                    var bottom = mEventRects!![i].bottom
                    bottom = mHourHeight.toFloat() * 24f * bottom / 1440 + mCurrentOrigin.y + mHeaderHeight + (mHeaderRowPadding * 2).toFloat() + mHeaderMarginBottom + mTimeTextHeight / 2 - mEventMarginVertical
    ...
    private fun sortEvents(events: List<WeekViewEvent>) {
        events.sortedWith(kotlin.Comparator { event1, event2 ->
            val start1 = event1.startTime?.timeInMillis ?: 0
            val start2 = event2.startTime?.timeInMillis ?: 0
            var comparator = if (start1 > start2) 1 else if (start1 < start2) -1 else 0
            if (comparator == 0) {
                val end1 = event1.endTime?.timeInMillis ?: 0
                val end2 = event2.endTime?.timeInMillis ?: 0
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        val gestured = mGestureDetector?.onTouchEvent(event) ?: false
    ...
    private fun forceFinishScroll(): Boolean {
        return mScroller != null && mScroller!!.currVelocity <= mMinimumFlingVelocity
    }
    ...
    private fun clipRect(canvas: Canvas, l: Float, t: Float, r: Float, b: Float) {
        canvas.save()
